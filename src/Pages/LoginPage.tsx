...
const Login: React.FC = () => {
  ...
  const onSubmit = async (data: LoginForm) => {
    try {
      ...
      if (res.token) {
        const isAdmin = res.is_admin === true;

        // Dispatch login with all fields
        dispatch(
          login({
            token: res.token,
            name: res.name,
        ...
        showToast(res.message || "Login successful!", true);
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-orange-400 text-white py-2 px-4 rounded-md hover:bg-orange-500 focus:outline-none disabled:opacity-50 flex items-center justify-center cursor-pointer transition"
          >
            {isLoading ? (
              <>
                <Loader2 className="animate-spin mr-2" size={20} />
